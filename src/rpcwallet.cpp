...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new EverGreenCoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current EverGreenCoin address for receiving payments to this account.");
...
Value stakeforcharity(const Array &params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        if (!changeAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid EverGreenCoin change address");
    }
    ...
    LOCK(pwalletMain->cs_wallet);
    {
        ...
        //For now max percentage is 50. EGC 100%
        if (nPer > 100 )
            nPer = 100;
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "Returns the total amount received by <evergreencoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid EverGreenCoin address: ")+s.name_);
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, false, strTxComment);
    if (!fCreated)
    {
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is an EverGreenCoin address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("evergreencoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("evergreencoin-lock-wa");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // evergreencoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; EverGreenCoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return information about <evergreencoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            ...
            "Return information about <evergreencoinpubkey>.");
...
// evergreencoin: reserve balance from being staked for network protection
...
// evergreencoin: check wallet integrity
...
// evergreencoin: make a public-private key pair
