...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "EverGreenCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "EverGreenCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EverGreenCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EverGreenCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
   ...
   // result.push_back(Pair("charityvalue", (int64_t)pblock->vtx[0].vout[0].nValue));  // EGC un
   // result.push_back(Pair("target", hashTarget.GetHex())); // EGC un
