...
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // every 10 blocks
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing;


int64_t devCoin = 1 * COIN; // 2% of block reward (50 * COIN) = 1 EGC per block to the EGC Foundation
int nCoinbaseMaturity = 50; // EGC mature in 60 blocks
...
const string strMessageMagic = "EverGreenCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // evergreencoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // evergreencoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+10) - GetDepthInMainChain()); // 10
}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * nRewardCoinYear / 365; // EGC removed " / COIN;"
...
// evergreencoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // evergreencoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // evergreencoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // evergreencoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // evergreencoin: track money supply and mint amount info
...
// evergreencoin: total coin age spent in transaction, in the unit of coin-days.
...
// evergreencoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // evergreencoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // evergreencoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // evergreencoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // evergreencoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 50; //coinbase maturity change to 60 blocks (1 hour)
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // evergreencoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // evergreencoin: check proof-of-stake
    ...
    // evergreencoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // evergreencoin: getblocks may not obtain the ancestor block rejected
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EverGreenCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "EGC to the tree top!";
        CTransaction txNew;
        txNew.nTime = 1448956800;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1448956800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4891360;
        if(fTestNet)
        {
            block.nNonce   = 19760907;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("201817b2cc654357f5491cd1c2c0553e7d2001a057e1fb0d31ce7c4259c1cbe5"));
        block.print();
        ...
        // evergreencoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x21, 0x24, 0x62, 0x47 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // evergreencoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // evergreencoin: tell downloading node about the latest block if it's
