EverGreenCoin-qt: Qt5 GUI for EverGreenCoin
===============================
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable evergreencoin experience), pass the following argument to qmake:
...
A warning for people using the *static binary* version of EverGreenCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of EverGreenCoin is linked against libdb 5.0 (see also `this Debian issue`_).
...
installed causes EverGreenCoin-Qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
